...
test.describe('Fitness Tracker E2E Tests', () => {
  ...
  test('should work in demo mode', async ({ page }) => {
    ...
    await page.goto('/log');
    await expect(page.locator('h1').first()).toContainText(/log/i);
  ...
  test('should handle API endpoints correctly', async ({ page }) => {
    ...
    // Accept either sqlite or demo as valid sources (demo is used in CI)
    expect(['sqlite', 'demo']).toContain(data.source);
  ...
  test('should show signup page', async ({ page }) => {
    ...
    // Check that either signup form exists or page redirects appropriately
    // Form may have different field types depending on auth configuration
    const emailInput = page.locator('input[type="email"], input[name="email"], input[placeholder*="email" i]').first();
    const passwordInput = page.locator('input[type="password"], input[name="password"]').first();

    // Check form elements exist (with increased timeout for CI)
    const hasEmailField = await emailInput.isVisible({ timeout: 10000 }).catch(() => false);
    const hasPasswordField = await passwordInput.isVisible({ timeout: 5000 }).catch(() => false);

    // Either both fields are visible, or page shows alternative auth UI
    const hasForm = hasEmailField && hasPasswordField;
    const hasAuthUI = await page.locator('text=/sign|create account|register/i').first().isVisible({ timeout: 5000 }).catch(() => false);
