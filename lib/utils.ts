...
/**
 * Get RPE (Rate of Perceived Exertion) description
 */
export function getRPEDescription(rpe: number): string {
  if (rpe <= 5) return 'Easy';
  if (rpe <= 6) return 'Moderate';
  if (rpe <= 7) return 'Somewhat Hard';
...
/**
 * Get intensity color class based on level
 */
export function getIntensityColor(intensity: 'low' | 'medium' | 'high' | string): string {
  switch (intensity) {
    case 'low':
...
/**
 * Get energy emoji based on rating (1-5)
 */
