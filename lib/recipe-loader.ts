...
export async function fetchRecipeIndex(): Promise<RecipeIndexEntry[]> {
  ...
  try {
    // Use API route instead of static file (Next.js standalone build issue)
    const response = await fetch(`${getBaseUrl()}/api/recipes-data/index`);
    if (!response.ok) {
...
export async function fetchRecipesByCategory(category: RecipeCategory): Promise<Recipe[]> {
  ...
  try {
    // Use API route instead of static file (Next.js standalone build issue)
    const response = await fetch(`${getBaseUrl()}/api/recipes-data/${category}`);
    if (!response.ok) {
