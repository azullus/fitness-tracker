 ...
 */
function seedDefaultData(_database: Database.Database): void {
  ...
  void _database; // Intentionally unused
  return;
}
...
export function getPantryItems(category?: string, lowStockOnly?: boolean): PantryItem[] {
  ...
  const rows = database.prepare(query).all(...params) as PantryItemRow[];
...
export function getDatabaseStats(): { path: string; size: number; tables: string[] } {
  ...
  // Use synchronous fs import - already imported at top of file
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const fs = require('fs');

  return {
    path: DB_PATH,
    size: existsSync(DB_PATH) ? fs.statSync(DB_PATH).size : 0,
